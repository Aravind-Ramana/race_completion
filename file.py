...
for i in range(0, len(data), group_size):
    ...
    sd, cd, s, lat, long = map(np.array, (batch[c] for c in data.columns.to_list()))

    new_step = sd.sum()
    relelevation = (sd * np.tan(np.radians(s))).sum()

    outdf.loc[row_counter] = [new_step, cd[-1], np.degrees(np.arctan(relelevation/new_step)), lat.mean(), long.mean()]
    row_counter += 1
